...
def generate_lagrange_basis(points):
    ...
    # 关键修复：将NumPy数组转换为Python原生浮点数列表
    py_points = [float(p) for p in points]
    ...
    for i in range(n):
        ...
        for j in range(n):
            if j != i:
                numerator *= x - py_points[j]
                denominator *= py_points[i] - py_points[j]
...
for i in range(0, nsp):
    for j in range(0, nsp):
        mmatrix[i, j] = float(integrate(basis[i] * basis[j], (x, -1, 1)))
print("Mass Matrix = \n", mmatrix)

inverse_mmatrix = np.linalg.inv(mmatrix)
print("Mass Matrix invert = \n", inverse_mmatrix)

# 关键修复：将节点数组转换为Python原生浮点数列表
x_lgl_py = [float(x) for x in x_lgl]  # 转换为Python原生类型
dmatrix = np.zeros((nsp, nsp))
for i in range(0, nsp):
    for j in range(0, nsp):
        dmatrix[i, j] = float(deri_basis[j].subs(x, x_lgl_py[i]))
print("Deritive Matrix invert = \n", dmatrix)

smatrix = np.zeros((nsp, nsp))
for i in range(0, nsp):
    for j in range(0, nsp):
        smatrix[i, j] = float(integrate(basis[i] * deri_basis[j], (x, -1, 1)))
print("S Matrix = \n", smatrix)

smatrix_md = np.dot(mmatrix, dmatrix)
print("S Matrix_md = \n", smatrix_md)

B_matrix = np.add(smatrix, smatrix.T)
print("B Matrix = \n", B_matrix)

m_entropy_matrix = np.diag(w_lgl)
print("S matrix entropy = \n", np.dot(m_entropy_matrix, dmatrix))
